...
`);
  ...
  if (await fs.pathExists(servicesPath)) {
    ...
    if (serviceClasses.length > 0) {
      ...
      serviceClasses.forEach(sc => console.log(`  - ${sc} (in ${path.relative(projectRoot, servicesPath)} )`));
    }
  } else {
...
Found potential API classes (@Service classes usually):`);
    ...
    console.log(" известных API Class directory not found: src/chrome/handlers/api");
  }
...
export function registerUpdateDepsCommand(program: Command) {
  const updateCommand = program.command('update').alias('u').description('Utility commands for updating project parts.');

  updateCommand
    .command('deps')
    .alias('d')
    .description('Checks and advises on TypeDI container registration for services, APIs, and DAOs.')
    .action(updateDeps);
}
