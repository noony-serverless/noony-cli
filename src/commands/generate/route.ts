...
const methodDetails: {
  [key: string]: { handlerName: string; httpMethod: string };
} = {
...
export function generateRoute(name: string, options: RouteOptions) {
  ...
  if (options.resource) {
    ...
    methodsToGenerate = (options.methods || defaultMethods)
      .split(',')
  ...
  const templateMethods = methodsToGenerate
    .map(methodKey => {
      const detail = methodDetails[methodKey];
      if (!detail) {
        console.warn(
          `Warning: Route method '${methodKey}' is not recognized. Skipping.`
        );
        return null;
      }
      return {
...
export function registerGenerateRouteCommand(program: Command) {
  program
    ...
    .option(
      '-m, --methods <methods>',
      'Comma-separated list of HTTP methods (getById,post,put,delete,getAll)'
    )
    .option(
      '--resource',
      'Generate RESTful resource routes (implies all CRUD methods)'
    )
    .option(
      '--auth <type>',
    ...
    .option(
      '-f, --feature <feature>',
      'Feature name, used to locate handlers (defaults to kebab-case <name>)'
    )
