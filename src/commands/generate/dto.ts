...
export function generateDto(name: string, options: DtoOptions) {
  ...
  try {
    ...
    logger.generated(targetFilePath, `${pascalCaseName} DTOs`);

    if (options.validation) {
      logger.info("Note: --validation flag was passed. You may need to manually add custom validation rules to the generated DTO file.");
    }
    if (options.nested) {
      logger.info("Note: --nested flag was passed. You may need to manually define nested DTO schemas in the generated file.");
    }
  } catch (e: any) {
    logger.error(`Failed to generate DTOs for '${name}': ${e.message}`);
  }
}
...
export function registerGenerateDtoCommand(program: Command) {
  program
    .command('dto <name>')
    .aliases(['dt', 'gdt'])
    .description('Generate new Data Transfer Object (DTO) files with Zod schemas for request/response validation (Base, New, Main, Update).')
    .option('-f, --fields <fields>', 'Comma-separated list of DTO fields and their types (e.g., "title:string,content:string,tags?:string[],author:AuthorDto"). Use "?" for optional fields. Custom types can be used for nested DTOs.')
    ...
    .addHelpText('after', `
Examples:
  noony generate dto post --fields "title:string,body:string,isPublished?:boolean"
  noony generate dto userProfile --fields "bio?:string,website:string,avatarUrl:string"`)
    .action(generateDto);
}
