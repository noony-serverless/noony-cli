...
`);
  ...
  const handlerActualFileDir = path.join(process.cwd(), getSrcPath(), 'chrome', 'handlers', featureName); // Same as targetDir for test
...
export function generateTest(type: string, name: string, options: TestOptions) {
  ...
  if (!options.unit && !options.e2e) {
    ...
    if (lcType === 'e2e') {
      ...
      console.warn(`No test type specified with --unit or --e2e, and type "${type}" is not a recognized component for default test generation. Please use --unit or --e2e.`);
      return;
    }
  }
  ...
  if (options.unit) {
    switch (lcType) {
      ...
      //   break;
      default:
        console.warn(`Unit test generation for type "${type}" is not currently supported. Supported types for --unit: 'handler'.`);
    }
  }
...
export function registerGenerateTestCommand(program: Command) {
  program
    ...
    .aliases(['gt', 't'])
    .description('Generate test files. Type: e.g., handler, service, or "e2e" for feature-wide tests.')
    .option('--unit', 'Generate unit tests for the specified <type> and <name>.')
    .option('--e2e', 'Generate E2E tests (.feature & .steps) for the specified <name> (used as feature name).')
    .option('-f, --feature <featureName>', 'Feature group name (used for handler unit tests path; defaults to <name>).')
    .option('-m, --methods <methodList>', 'Comma-separated list of methods for handler unit tests (e.g., "get,post").')
    .action(generateTest);
}
