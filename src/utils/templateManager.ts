...
function getCustomTemplatePath(
  templateType: keyof NoonyRcTemplates
): string | undefined {
...
export function getTemplateContent(
  ...
  const builtInTemplateFullPath = path.join(
    __dirname,
    '../templates',
    defaultBuiltInPath
  ); // Adjusted to standard CLI structure
  ...
  if (customPathString) {
    ...
    if (fs.existsSync(userCustomPath)) {
      try {
        console.log(
          `Using custom template for '${templateType}' from: ${userCustomPath}`
        );
        return fs.readFileSync(userCustomPath, 'utf-8');
      } catch (error) {
        console.warn(
          `Warning: Error reading custom template '${userCustomPath}'. Falling back to built-in. Error: ${error}`
        );
      }
    } else {
      console.warn(
        `Warning: Custom template for '${templateType}' not found at '${userCustomPath}'. Falling back to built-in template.`
      );
    }
  }
  ...
  try {
    ...
    console.error(
      `FATAL: Built-in template '${defaultBuiltInPath}' not found at '${builtInTemplateFullPath}'. Error: ${error}`
    );
